...
fn main() {
    ...
    let tasks = db.get_tasks_with_filter(None, None, None).expect("Failed to get tasks");
    println!("  Count: {}", tasks.len());
    for task in &tasks {
        println!("  - [{}] {:?} {}", if task.completed { "x" } else { " " }, task.priority, task.title);
    }
    
    println!("\nIncomplete tasks only:");
    let incomplete = db.get_tasks_with_filter(None, Some(false), None).expect("Failed to get incomplete tasks");
